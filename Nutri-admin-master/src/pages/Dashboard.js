...
const columns = [
  ...
  {
    title: "Total Price With Delivery",
    dataIndex: "dprice",
  },
  {
    title: "Status",
...
const config3 = {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < orderState?.length ; i++) {
      data1.push({
        key: i,
        name: `${orderState[i].user.firstname} ${orderState[i].user.lastname}`,  // Ensure space between first and last name
        product: orderState[i].orderItems?.length,
        price: `KES ${orderState[i]?.totalPrice?.toLocaleString()}`,  // Add currency tag and apply locale string formatting
        dprice: `KES ${orderState[i]?.totalPriceAfterDiscount?.toLocaleString()}`,  // Add currency tag and apply locale string formatting
        ...
        <div className="d-flex p-3 justify-content-between align-items-end flex-grow-1 bg-white p-3 roudned-3">
          <div>
            ...
            <h4 className="mb-0 sub-title">
              KES {yearlyDataState && yearlyDataState[0]?.amount?.toLocaleString()}
            </h4>
          </div>
