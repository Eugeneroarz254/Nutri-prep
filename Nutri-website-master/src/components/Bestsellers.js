...
const BestSellers = () => {
  ...
  const isTitleTooLong = productState?.some(item => countWords(item?.title) > 5);
    ...
    <>
      <Container class1="featured-wrapper home-wrapper-2">
        ...
        <div className={`ranking-collection flex-column d-flex${isDesktop ? ' featured' : ''}`}>
          {productState &&
            productState
              .filter((item) => item.tags === "Best Sellers")
              .slice(0, 3)
              .map((item, index) => {
                  ...
                  <div key={index} className={"col-3 px-0"}>
                    ...
                    >
                      ...
                      <div className="product-details">
                        ...
                        <h5 className={`product-title ${countWords(item?.title) > 5 ? 'ellipsis' : ''}`}>
                        {item?.title}
                      </h5>
