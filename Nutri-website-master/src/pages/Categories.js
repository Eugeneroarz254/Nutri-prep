...
const formatCategoryName = (category) => {
    if (!category) return ''; // Handle null or undefined case
    return category
      .split('-')
  ...
  const { type, category: categoryFromUrl } = useParams();  // Extract category from URL
  
  // Format the category name
  const formattedType = formatCategoryName(type);
  const formattedCategory = formatCategoryName(categoryFromUrl);
...
useEffect(() => {
  ...
  for (let index = 0; index < productsState.length; index++) {
    ...
    if (element.category === formattedCategory) {  
      data.push(element);
    }
  }

  setPopularProduct(data);
}, [productsState, categoryFromUrl]);
...
const [tempMaxPrice, setTempMaxPrice] = useState(maxPrice || "");
  ...
  console.log(formattedCategory,"category")

  return (
    <>
     <Meta 
      title={`${formattedCategory} - Synergy Views`}
    ...
    />
     ...
     <CategoryBreadcrumb title={formattedCategory} />
        ...
        return (
          <li
            ...
            style={{
              color:
                isActive || hoveredCategory === formattedCategory
                  ? "#b73c3c" // Apply hover color or active color
                  : "inherit",
              cursor: "pointer",
              transition: "color 0.3s", // Smooth transition effect
            }}
